...
type MapReduce struct {
    ...
    finishMap chan string
    finishReduce chan string
}
...
func InitMapReduce(nmap int, nreduce int,
	...
	mr.finishMap = make(chan string)
	mr.finishReduce = make(chan string)
...
func MakeMapReduce(nmap int, nreduce int,
	file string, master string) *MapReduce {
	log.Println("MakeMapReduce", nmap, nreduce)
...
func (mr *MapReduce) Register(args *RegisterArgs, res *RegisterReply) error {
	log.Println("master register", args.Worker)
...
func (mr *MapReduce) CleanupRegistration() {
	...
	if ok == false {
		log.Printf("Cleanup: RPC %s error\n", mr.MasterAddress)
	}
