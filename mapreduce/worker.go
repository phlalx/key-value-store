...
func (wk *Worker) DoJob(arg *DoJobArgs, res *DoJobReply) error {
  log.Printf("Dojob %s job %d file %s operation %v N %d\n",
...
func (wk *Worker) Shutdown(args *ShutdownArgs, res *ShutdownReply) error {
  log.Print("Worker", wk.name, "shutdowns")
  DPrintf("Shutdown %s\n", wk.name)
  res.Njobs = wk.nJobs
  res.OK = true
...
func Register(master string, me string) {
    log.Print("Worker", me, "Registers master", master)
  args := &RegisterArgs{}
  args.Worker = me
...
func RunWorker(MasterAddress string, me string,
  ...
  log.Println("Worker", me, "RunWorker master", MasterAddress)
  DPrintf("RunWorker %s\n", me)
  wk := new(Worker)
  wk.name = me
  wk.Map = MapFunc
